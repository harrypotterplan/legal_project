...
# ── 경로 설정 ─────────────────────────────────────────────────────────
# 실행 위치와 상관없이 backend 폴더 기준으로 모델/ChromaDB 경로를 잡기 위함
...
#
# 변경 이유:
# "./jurisim-sbert-v4"는 터미널 실행 위치 기준 상대경로임.
# BASE_DIR 기준으로 바꾸면 항상 backend/jurisim-sbert-v4를 찾음.
...
# ── 판례 ChromaDB ─────────────────────────────────────────────────────
# 기존 코드:
# CASE_DB_PATH = "./chroma_db"
#
# 변경 이유:
# 실행 위치가 바뀌어도 backend/chroma_db를 안정적으로 찾기 위함.
...
# ── 법령 ChromaDB ─────────────────────────────────────────────────────
# 기존 코드:
# LAW_DB_PATH = "./chroma_db_law"
#
# 변경 이유:
# 실행 위치가 바뀌어도 backend/chroma_db_law를 안정적으로 찾기 위함.
...
def normalize_with_gemini(query: str) -> str:
    ...
    try:
        ...
        response = gemini.generate_content(
            f"""아래 문장을 법률 검색에 적합한 표준 법률 용어로 변환하세요.
...
출력:""",
        ...
        # 변경 추가:
        # 정규화 결과가 너무 짧으면 잘린 문장일 가능성이 있으므로 원문 사용
        if (
            len(normalized) < 25
...
def calc_recency(date_str: str) -> float:
    """선고일자 기준 최신성 점수 산출 (30년 기준 선형 감소)"""
    try:
...
def run_pipeline(query: str, category: str):
    ...
    normalized_query = normalize_with_gemini(query)

    # 변경 추가:
    # 정규화 문장이 원문 의미를 일부 잃을 수 있어 원문도 함께 검색에 사용
    ...
    #
    # 변경 이유:
    # 판례가 없다는 이유로 법령 검색 결과까지 버려지는 것을 막기 위함.
