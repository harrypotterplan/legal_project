...
// ==================== Main Component ====================
// ✨ 1. App.jsx에서 넘겨준 고정 모드 Props 받기
const ChatSection = ({ setAiResult, forceLowScore, setForceLowScore }) => {
  ...
  const handleReset = async () => {
    if (window.confirm("현재 대화를 지우고 새로운 시뮬레이션을 시작하시겠습니까?")) {
      try {
        ...
        // ✨ 2. 새 시뮬레이션 시작 시 45% 고정 모드도 같이 해제
        if (setForceLowScore) {
          setForceLowScore(false);
        }
      } catch (error) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // ✨ 3. [데이터 가로채기] 백엔드 응답 데이터를 복사
      const finalData = { ...response.data };
      
      // ✨ 4. 강제 45% 모드가 켜져있다면 점수를 45로 조작하고 모드 즉시 해제
      if (forceLowScore) {
        finalData.reliability_score = 45;
        if (setForceLowScore) setForceLowScore(false); 
      }

      const aiAnswerText = finalData.answer;
      ...
      if(setAiResult) {
        setAiResult(finalData); // ✨ 조작된(또는 원본) 데이터를 App.jsx로 전송
      }
